...
public class GlitchSerie implements Plottable{
    ...
    public GlitchSerie(String name) {
        ...
        this.name = name;
        this.col=SerieType.getDefColor(SerieType.xy);
        this.lineWidth=SerieType.getDefLineWidth(SerieType.xy);
        this.markerSize=SerieType.getDefMarkerSize(SerieType.xy);
        this.plotThis=true;
    }

    public void addPoint(Gpoint gp) {
        this.points.add(gp);
    }
    ...
    public Plottable_point getScaledPoint(int index, float scaleFactor) {
        return new Gpoint((Gpoint)points.get(index),scaleFactor); 
    }
